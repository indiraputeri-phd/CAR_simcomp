...
source("scripts/scripts on spdata experiment/00e_sarcar_func.R")
...
for (m in 1:M) {
  gen_data_list[[m]] <- sarcarsp_dat(gen_dat = gen.dat,
                                     artfcountry, rho, tau, sigma,
                                     beta_values = true_params,
                                     num_repetitions = 128
  )
}
